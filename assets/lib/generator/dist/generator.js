...
Utils.$document.ready(function () {
  ...
  btnDownload.on('click', function () {
    btnDownload.attr('href', "data:text/plain;charset=utf-8, " + html_beautify(html().html, {
      indent_size: 2,
      indent_with_tabs: false
    }));
...
Utils.$window.on('load', function () {
  ...
  |   Filter
  -----------------------------------------------*/
  // const filter = (basis = '*', target = []) => {
  //   console.log(basis);
  //   if (basis === '*') {
  //     target.map(item => $(target[item]).removeClass('d-none'));
  //     return;
  //   }
  //   $.each(target, function( index, value ) {
  //     const $this = $(value); 
  //     $this.hasClass(basis) ? $this.removeClass('d-none') : $this.addClass('d-none')
  //   });
  //   // target.map(item => {
  //   //   if ($(target[item]).hasClass(basis)){
  //   //     $(target[item]).removeClass('d-none');
  //   //   } else {
  //   //     $(target[item]).addClass('d-none');
  //   //   }
  //   //   console.log($(target[item]));
  //   //   // $(target[item]).hasClass(basis) ? $(target[item]).removeClass('d-none') : $(target[item]).addClass('d-none')
  //   // });
  // };
  // $('#generator-menu').on('click', '.menu-item', (e) => {
  //   const navItem = $(e.target);
  //   const filterItem = navItem.data('filter');
  //   navItem.addClass('active');
  //   navItem.siblings('.active').removeClass('active');
  //   filter(filterItem, draggableItemsCollection);
  // });
